...
export default function MicaIndicators({ micaIndicators, lastUpdated }: Props) {
  ...
  const description = (
    <>
      The EU Markets in Crypto-Assets (MiCA) regulation requires token issuers
      and crypto-asset service providers (CASPs) to disclose sustainability
      data. In partnership with the{' '}
      <Link href="https://carbon-ratings.com/">
        Crypto Carbon Rating Institute
      </Link>{' '}
      (CCRI) , you can find the methodology for the data below{' '}
      <Link href="/mica-indicators/methodology">here</Link>.
    </>
  )
