...
export function HostItem({
  ...
  const rate = useActiveCurrencySiascanExchangeRate({})
  const storageCost = useMemo(
    () =>
      `${rate.currency} ${new BigNumber(host.settings.storageprice)
        .times(TBToBytes(1))
  ...
  const downloadCost = useMemo(
    () =>
      `${rate.currency} ${new BigNumber(host.settings.downloadbandwidthprice)
        .times(TBToBytes(1))
        .div(1e24)
        .times(rate.rate || 1)
        .toFixed(2)}/TB`,
  ...
  const uploadCost = useMemo(
    () =>
      `${rate.currency} ${new BigNumber(host.settings.uploadbandwidthprice)
        .times(TBToBytes(1))
        .div(1e24)
        .times(rate.rate || 1)
        .toFixed(2)}/TB`,
