...
export default async function Page({ params }) {
  ...
  const [formationTxnID, finalRevisionTxnID] =
    getFirstAndLastRevisionsTransactionIDs(contract, previousRevisions)
...
function getFirstAndLastRevisionsTransactionIDs(
  contract: ExplorerFileContract,
  previousRevisions: ExplorerFileContract[] | undefined
) {
  if (!previousRevisions || !previousRevisions.length)
    return [contract.transactionID, contract.transactionID]

  return [
    previousRevisions[0].transactionID,
