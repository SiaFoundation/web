...
export function EntityListItem(entity: EntityListItemProps) {
  const sc = entity.sc
  const sf = entity.sf
  const maturityHeight = entity.maturityData?.maturityHeight ?? 0
  const currentHeight = entity.maturityData?.currentHeight ?? 0
  const isMature = maturityHeight <= currentHeight
