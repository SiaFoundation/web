...
import { scDecimalPlaces, SettingsData } from './meta'
...
export function transformUp(
  ...
  return {
    ...
    windowSize: Number(values.windowSize.times(daysToBlocks(1)).toFixed(0)),
    ...
    ingressLimit: Number(values.ingressLimit.toFixed(0)),
    egressLimit: Number(values.egressLimit.toFixed(0)),
...
export function transformDown(s: HostSettings): SettingsData {
  return {
    ...
    windowSize: new BigNumber(s.windowSize).div(daysToBlocks(1)),
    ...
    // Bandwidth limiter settings
    ingressLimit: new BigNumber(s.ingressLimit),
    egressLimit: new BigNumber(s.egressLimit),

    // DNS settings
    dnsProvider: s.dynDNS.provider,
    dnsIpv4: s.dynDNS.ipv4,
    dnsIpv6: s.dynDNS.ipv6,
    dnsOptions: s.dynDNS.options,
  }
}
